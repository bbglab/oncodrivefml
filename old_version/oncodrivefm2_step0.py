...
def _load_mutations(project, tissue):
    if tissue != 'pan':
        mutations_file = os.path.join(sampling.INPUT_FOLDER, "datasets", project, "{}.txt".format(tissue))
        ...
        mutations_files = glob.glob(os.path.join(sampling.INPUT_FOLDER, "datasets", project, "*.txt"))
        if project == "intogen":
            mutations_files = [f for f in mutations_files if f.find("tcga") != -1]
        muts = pd.read_csv(mutations_files[0], sep='\t',
                           dtype={'CHROMOSOME': object, 'POSITION': np.int, 'REF': object, 'ALT': object,
                                  'SAMPLE': object, 'TYPE': object})
        muts['PROJECT'] = os.path.basename(mutations_files[0]).replace('.txt', '')
        for mutations_file in mutations_files[1:]:
            more_muts = pd.read_csv(mutations_file, sep='\t',
                                    dtype={'CHROMOSOME': object, 'POSITION': np.int, 'REF': object, 'ALT': object,
                                           'SAMPLE': object, 'TYPE': object})
            more_muts['PROJECT'] = os.path.basename(mutations_file).replace('.txt', '')
...
def _load_features(feature):
    regions_file = os.path.join(sampling.INPUT_FOLDER, "features_regions", "{}.regions".format(feature))
